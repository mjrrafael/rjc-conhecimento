# Achados e pendências

- `data/benefits_crosswalk.json` tinha 9.726 cards sem prova por campo; foi externalizado e está com zero entradas públicas.
- `data/benefits_quarantine.json` tinha 13.150 itens materiais; foi externalizado e está com zero entradas públicas.

## Pendências

- Revalidar cada benefício a partir de fonte primária, com recibos nativos e revisão independente, antes de qualquer nova publicação. Status: BLOQUEADO.
- Tratar histórico, forks, caches e demais acervos legados como superfícies ainda a conter.
