...
def audit_content_pages() -> list[str]:
    ...
    for uf in STATE_NAMES:
        ...
        if uf == "MT":
            index_html = read_page(state_index_path(uf))
            if not index_html or "Mato Grosso: ICMS e benefícios fiscais em tela" not in index_html:
                errors.append("MT: indice profundo de ICMS e beneficios ausente")
            state_html = read_page("estados/mt.html")
            if not state_html or "Benefícios fiscais por grupo" not in state_html:
                errors.append("MT: pagina estadual sem grupos didaticos de beneficios")
            if state_html and re.search(r"\bTaxas\b|MT_ITCD|MT_OUTROS", state_html):
                errors.append("MT: pagina estadual ainda contem inventario antigo ou taxas fora de escopo")
            for chapter in MT_CHAPTERS:
                chapter_html = read_page(mt_chapter_path(chapter["id"]))
                if not chapter_html or "Texto legal antes da análise" not in chapter_html or "law-pre" not in chapter_html:
                    errors.append(f"MT: capitulo sem lei em tela: {chapter['id']}")
            benefits_html = (
                read_page(mt_chapter_path("beneficios-matriz-lc160"))
                + read_page(mt_chapter_path("prodeic-desenvolvimento"))
                + read_page(mt_chapter_path("isencoes-reducoes-creditos"))
                + read_page(mt_chapter_path("agro-cesta-diferimento"))
                + read_page(mt_chapter_path("documentos-cbenef-efd-prova"))
            )
